...
def arming_state(df: pd.DataFrame, regime_by_date) -> dict | None:
    ...
    for tr in r3:
        ...
        if sess != sess_now:                    # arming window belongs to a past session
            continue
        if c < ki + 2:                          # not yet armed (need stop_bar+2)
            continue
        ...
        best = dict(
            ...
            buy_stop=round(runmax, 2), prospective_stop=round(prospective_stop, 2),
...
def bracket_on_fill(df: pd.DataFrame, regime_by_date, fill_px: float,
                    signal_time_iso: str, session: str) -> dict:
    ...
    lag_vw = float(vwap5[c]); lag_up = float(upper5[c])
    tp_kind = "VWAP" if fill_px < lag_vw else "UPPER"
    target = lag_vw if tp_kind == "VWAP" else lag_up
    return dict(stop=round(stop_lvl, 2), tp_kind=tp_kind, target=round(float(target), 2),
                riskpt=round(fill_px - stop_lvl, 2))
