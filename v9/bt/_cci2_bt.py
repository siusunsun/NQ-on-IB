"""_cci2_bt.py - regenerate CCI trades for AM + PM under DIFFERENT PM windows, to test
kristov18's live two-slot spec (PM 12:00-14:00 k=1.5) vs our old PM (13:00-15:00 k=1.5)
on OUR data. Read-only research. Writes scratch _cci2_trades.csv (tagged pmwin). No live code."""
import sys, glob
...
def main():
    ...
    # slot defs: AM fixed; PM tested under his-live-window and our-old-window
    SLOTDEFS=[
        (9*60+30, 12*60, 1.0, 'AM'),          # AM 09:30-12:00 k=1.0
        (12*60,   14*60, 1.5, 'PM_his'),      # his live spec 12:00-14:00 k=1.5
        (13*60,   15*60, 1.5, 'PM_old'),      # our old _cci_bt.py 13:00-15:00 k=1.5
    ]
    trades=[]
    for dk in sorted(daymap.keys()):
        rows=daymap[dk]
        for (w0,w1,kstop,slotname) in SLOTDEFS:
    ...
    T.to_csv('/root/v9/bt/_cci2_trades.csv',index=False)
    print("rows:",len(T),"slots:",T['slot'].value_counts().to_dict())
main()
