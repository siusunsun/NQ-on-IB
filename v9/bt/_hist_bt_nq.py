...
NQ_DIR = Path("/root/tlbrc_paper/ref/data_full/NQ")
...
def load_all():
    ...
    df = pd.concat([load_csv(f) for f in files]).sort_index(kind="mergesort")  # 2026-09-14: STABLE sort (see bt_nq.py)
...
if __name__ == "__main__":
    main()
# appended by _hist_ research: fast, behavior-identical sim_vwap (searchsorted instead of O(n) slice)
_FAST = {}
def _fast_arrays(df):
    key = id(df)
    if key not in _FAST:
        et = df.index.tz_convert(ET)
        etmin = np.array([t.hour * 60 + t.minute for t in et], dtype=np.int32)
        ethour = np.array([t.hour for t in et], dtype=np.int32)
        _FAST.clear()
        _FAST[key] = (df.index, df["high"].values, df["low"].values, df["close"].values, etmin, ethour)
    return _FAST[key]
...
def sim_vwap(df, sig, entry_utc):
    ...
    entry = sig["entry_price"]; stop = sig["stop_price"]; target = sig["target_price"]
    risk = abs(entry - stop)
    idx, hi, lo, cl, etmin, ethour = _fast_arrays(df)
    p0 = idx.searchsorted(entry_utc, side="left")
    for k in range(p0, len(idx)):
        if direction == "long":
            hit_stop = lo[k] <= stop; hit_tgt = hi[k] >= target
        else:
            hit_stop = hi[k] >= stop; hit_tgt = lo[k] <= target
        if hit_stop:
            return _mk_trade("VWAP", direction, entry_utc, entry, idx[k], stop, "STOP", risk)
        if hit_tgt:
            return _mk_trade("VWAP", direction, entry_utc, entry, idx[k], target, "TARGET", risk)
        if (etmin[k] >= 955) and (ethour[k] < 17):
            return _mk_trade("VWAP", direction, entry_utc, entry, idx[k], cl[k], "GAP_FLATTEN", risk)
    return _mk_trade("VWAP", direction, entry_utc, entry, idx[-1], cl[-1], "DATA_END", risk)
